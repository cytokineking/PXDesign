...
Key results are aggregated in the `<out_dir>/results/<task_name>/` folder (or `results_v2/`, `results_v3/`, ... if you finalize again in the same `<out_dir>`). Go to this folder and open `summary.csv` to see your ranked binders.
...
If you want to re-run the **final ranking/selection** (generate a new `results_vN/` snapshot) without re-running diffusion/eval, you can run:
