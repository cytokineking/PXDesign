...
PXDesign runs are **resumable**: re-run the same command with the same `<out_dir>` and it will only do missing work (and skip already-completed stages when possible).
...
**resume after interruption**. PXDesign can stream diffusion outputs incrementally to disk (atomic writes):
...
The primary results are located in `design_outputs/<task_name>/`. Raw diffusion/eval artifacts are organized per-run under `runs/run_XXX/`.
...
│   │   └── final/{all_summary.csv,filtered_summary.csv}
│   └── run_001/...
│
└── design_outputs/
    └── <task_name>/             <-- PRIMARY RESULTS FOLDER
        ...
        │
        ├── orig_designed/              <-- Designed backbones (copied from diffusion outputs)
...
If you want to re-run the **final ranking/selection** (regenerate `design_outputs/`) without re-running diffusion/eval, you can run:
`pxdesign rank --dump_dir <out_dir>`
