...
class ColabFoldMSAConfig:
    ...
    user_agent: str = "pxdesign"


def _a3m_first_sequence(a3m_text: str) -> Optional[str]:
    """
    Return the first sequence in an A3M (ungapped) or None if not found.
    """
...
def _fetch_colabfold_unpaired_a3m(
    *,
    ...
    """
    Fetch an unpaired MSA (A3M) for a single protein sequence via the ColabFold MMseqs2 API.

    This is a small, self-contained client inspired by ColabFold's reference implementation
    (and the one vendored in Protein Hunter).
    """
    try:
        import requests
        ...
        raise RuntimeError(
            "Missing dependency for ColabFold MSA generation. Please install `requests`."
        ) from e
    ...
    if has_basic_auth and has_header_auth:
        raise ValueError(
            "Cannot use both basic authentication (username/password) and header authentication."
        )
    ...
    # Submit (with retries for transient failures)
    ...
    # Handle immediate server statuses
    ...
    # Merge lines for query_id similarly to ColabFold reference logic
    a3m_lines: list[str] = []
    for p in a3m_files:
        update_m = True
        m = None
...
def ensure_pxdesign_msa_dir_from_colabfold(
    *,
    sequence: str,
    out_dir: Path,
    cfg: ColabFoldMSAConfig,
    ...
    """
    Ensure `out_dir` contains PXDesign-compatible MSA files:
      - non_pairing.a3m
      - pairing.a3m
    ...
    # PXDesign expects both files to exist. For single-chain MSAs, a paired alignment
    # isn't meaningful, so we write the same A3M to both.
    non_pairing.write_text(msa_text)
    pairing.write_text(msa_text)

    # Best-effort cleanup of large intermediates to keep output folders tidy.
